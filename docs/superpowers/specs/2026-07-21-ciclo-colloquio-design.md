...
5. **Guardrail su ogni risposta libera (S3) + filtro PII in uscita.** La risposta della persona passa dal `ScopeGuard`; se fuori-tema/injection → rifiuto gentile e **ri-pongo la stessa domanda** (il colloquio non deraglia). I testi **generati dall'LLM** e mostrati alla persona (riepiloghi, chiarimenti) passano dal **filtro PII** (`PiiRedactor`, Presidio, nessuna chiamata LLM) **prima di essere mostrati** (§7.3 «prima di mostrare»). *Perché §2/§7.3.* *(Follow-up tracciato: l'`output ScopeGuard` — `check_output`, una chiamata LLM — sui testi generati non è ancora applicato; rischio basso perché i testi derivano da dati solo-lavoro già validati, mostrati alla stessa persona nella stessa sessione. Le domande base sono template i18n statici e non richiedono filtro.)*
