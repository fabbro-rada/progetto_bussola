...
def find_section_clarification(
    ...
    prompt = (
        ...
        "Your DEFAULT is to NOT ask anything: the person will review and correct "
        "everything at the section summary and again at the final recap, so a "
        "clarification is a RARE exception, not a routine step. Ask ONLY in a "
        "genuinely doubtful case where the extracted data is UNUSABLE or "
        "SELF-CONTRADICTORY, i.e. ONLY when one of these clearly holds: "
        "(a) a role or skill was captured as a bare generic verb with no real job/skill "
        "('lavorato', 'fatto') so it is meaningless; (b) a number/duration is impossible "
        "or nonsensical; (c) a DIRECT contradiction with an earlier section — two facts "
        "that cannot both be true. "
        "Do NOT ask to get more detail, to enrich or confirm a plausible answer, to tidy "
        "wording, or because an answer is short, informal, or a MISSING/EMPTY field (the "
        "profile is intentionally minimal). A short, plain, understandable answer is FINE "
        "as-is. If you are not clearly in one of cases (a)–(c), set needs_clarification=false. "
        "If (and only if) you must ask, write ONE short, gentle, "
