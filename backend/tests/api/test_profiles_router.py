...
def test_search_profiles_is_audited(client, make_operator, app_conn: psycopg.Connection):
    # §6/§7.3: bulk profile search is the most obvious misuse vector, so it must
    # be traceable in the audit log — without leaking any profile/PII data.
    ProfileRepository(app_conn, PiiRedactor()).save(
        WorkProfile(
            pseudonym_id="P-1",
            skills=[Skill(name="Cucina", kind=SkillKind.TECHNICAL, evidence=EvidenceGrade.STATED)],
    ...
    with app_conn.cursor() as cur:
        cur.execute(
            ...
            "WHERE action = 'profiles_searched' ORDER BY id DESC LIMIT 1"
        )
        row = cur.fetchone()
    assert row is not None
    actor, target_pseudonym, details = row
    assert actor == user
    assert target_pseudonym is None
    assert set(details) <= {"filters", "results"}
    assert details == {"filters": "skill_query", "results": "1"}
